...
- 새로 출제할 때 문항 수와 단원, 문항당 제한 시간을 설정할 수 있습니다. 제한 시간 기본값은 10초입니다.
- 미응답 문항에서만 타이머가 동작하고, 이미 답한 문항에서는 타이머가 멈춥니다.
- 시간이 지나면 해당 문항은 시간 초과 오답으로 처리되지만, 다시 O/X를 선택해 답을 바꿀 수 있습니다.
- 단축키는 `←`/`→`로 이전/다음 문제, `z` 또는 `o`로 O, `x`로 X를 선택합니다.
